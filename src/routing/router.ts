...
export class UnifiedRouter {
	 ...
	 */
	async authenticate(request: Request, env: any, auth: ResolvedRouteAuthConfig): Promise<AuthValidationResult> {
	 ...
	 * Handle auth failure
	 */
	private handleAuthFailure(authResult: AuthValidationResult, auth: ResolvedRouteAuthConfig): Response {
		// Stealth mode
		if (auth.stealthMode) {
