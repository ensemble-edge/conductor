...
export class ProjectValidator {
	 ...
	 */
	private async validateMember(memberName: string): Promise<Pick<ValidationResult, 'errors' | 'warnings'>> {
		const errors: ValidationError[] = [];
		const warnings: ValidationWarning[] = [];
	 ...
	 */
	private async validateEnsemble(ensembleFile: string): Promise<Pick<ValidationResult, 'errors' | 'warnings'>> {
		const errors: ValidationError[] = [];
		const warnings: ValidationWarning[] = [];
