        ...
        try {
          ...
          if (options.json) {
            ...
            records.forEach((record) => {
              ...
              const name = (
                record.name.length > nameWidth - 3
                  ? record.name.substring(0, nameWidth - 3) + '...'
                  : record.name
              ).padEnd(nameWidth)
              const type = record.type.padEnd(typeWidth)
              const status = (
                record.status === 'success' ? chalk.green(record.status) : chalk.red(record.status)
              ).padEnd(statusWidth + 9) // +9 for color codes
              const duration = `${record.duration}ms`.padEnd(durationWidth)
              const time = new Date(record.startTime).toLocaleString()

              console.log(
                `${chalk.dim(id)}${name}${chalk.dim(type)}${status}${chalk.dim(duration)}${chalk.dim(time)}`
              )
            })

            console.log('')
            console.log(
              chalk.dim(`Showing ${records.length} execution${records.length !== 1 ? 's' : ''}`)
            )
            console.log('')
