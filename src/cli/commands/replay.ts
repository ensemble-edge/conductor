      ...
      async (
        ...
        try {
          ...
          if (!options.json) {
            ...
            console.log(
              chalk.dim(
                `Original status: ${record.status === 'success' ? chalk.green(record.status) : chalk.red(record.status)}`
              )
            )
            ...
            if (options.stepByStep) {
              console.log(
                chalk.yellow('⏸  Step-by-step mode enabled (press Enter to continue each step)')
              )
              console.log('')
            }
          }
          ...
          if (options.json) {
            ...
            if (record.steps && record.steps.length > 0) {
              ...
              if (options.fromStep) {
                ...
                if (stepIndex === -1) {
                  console.log(
                    chalk.yellow(`⚠ Step "${options.fromStep}" not found, showing all steps`)
                  )
                  console.log('')
                } else {
