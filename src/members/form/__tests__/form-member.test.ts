...
describe('FormMember', () => {
	...
	describe('Configuration Validation', () => {
		...
		it('should throw error if both fields and steps provided', () => {
			...
			expect(() => new FormMember(config)).toThrow(
				'Form member cannot have both fields and steps'
			);
		});
	...
	describe('Form Validation (mode=validate)', () => {
		it('should validate required fields', async () => {
			...
			expect(result.errors?.some((e) => e.field === 'name' && e.rule === 'required')).toBe(
				true
			);
			expect(result.errors?.some((e) => e.field === 'email' && e.rule === 'required')).toBe(
				true
			);
		});
