...
export class QueueMember extends BaseMember {
	private queueConfig: QueueMemberConfig;

	constructor(config: MemberConfig) {
		super(config);
		this.queueConfig = config as MemberConfig & QueueMemberConfig;
	 ...
	 */
	protected async run(context: MemberExecutionContext): Promise<QueueMemberOutput> {
	 ...
	 */
	private getQueue(context: MemberExecutionContext): any {
		const queueName = this.queueConfig.queue;
		const queue = context.env[queueName as keyof typeof context.env];

		if (!queue) {
			throw new Error(
				`Queue binding "${queueName}" not found. Add it to wrangler.toml`
			);
		}
