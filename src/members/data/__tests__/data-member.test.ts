...
import type { DataConfig, DataInput } from '../../data-member.js';
...
describe('DataMember', () => {
	...
	describe('GET Operation', () => {
		...
		it('should throw error if key is missing', async () => {
			...
			).rejects.toThrow('GET operation requires "key"');
		});
	});
	...
	describe('PUT Operation', () => {
		...
		it('should throw error if key or value is missing', async () => {
			...
			// Missing value
			await expect(
				member['run']({ ...mockContext, input: { key: 'test' } })
			).rejects.toThrow('PUT operation requires "key" and "value"');

			// Missing key
			await expect(
				member['run']({ ...mockContext, input: { value: 'data' } })
			).rejects.toThrow('PUT operation requires "key" and "value"');
		});
	});
	...
	describe('DELETE Operation', () => {
		...
		it('should throw error if key is missing', async () => {
			...
			await expect(
				member['run']({ ...mockContext, input })
			).rejects.toThrow('DELETE operation requires "key"');
		});
	});
