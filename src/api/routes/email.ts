...
/**
 * Email message structure from Cloudflare Email Routing
 */
 ...
 */
async function parseEmailFromRequest(c: any): Promise<{
	from: string
	to: string
	subject: string
	body: string
 ...
 * Find ensemble that handles this email address
 */
async function findEnsembleForEmail(emailAddress: string, env: Env): Promise<EnsembleConfig | null> {
 ...
 */
async function authenticateEmailSender(from: string, whitelist: string[]): Promise<boolean> {
 ...
 */
function formatOutputAsText(output: unknown): string {
 ...
 */
function formatOutputAsHtml(output: unknown): string {
