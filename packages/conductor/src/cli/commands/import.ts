 ...
 */
async function extractTarGz(archivePath: string): Promise<ExtractedFile[]> {
 ...
 */
async function extractZip(archivePath: string): Promise<ExtractedFile[]> {
  const files: ExtractedFile[] = []
 ...
 */
async function detectFormat(archivePath: string): Promise<'tar' | 'zip'> {
...
async function checkConflict(
  targetPath: string,
  newContent: Buffer
): Promise<{ exists: boolean; different: boolean }> {
  try {
...
export function createImportCommand(): Command {
  ...
  importCmd
    ...
    .action(async (bundlePath: string, options: ImportOptions) => {
      try {
        ...
        const manifest: BundleManifest = JSON.parse(manifestFile.content.toString('utf-8'))
        ...
        for (const file of filesToImport) {
          ...
          if (!exists) {
            ...
            console.log(
              `  ${typeIcon} ${chalk.green('+')} ${file.path}`,
              chalk.dim(`→ ${path.relative(process.cwd(), targetPath)}`)
            )
            ...
            console.log(
              `  ${typeIcon} ${chalk.yellow('!')} ${file.path}`,
              chalk.yellow('(overwritten)')
            )
            ...
            console.log(
              `  ${typeIcon} ${chalk.cyan('-')} ${file.path}`,
              chalk.cyan('(skipped - exists)')
            )
            results.push({ path: targetPath, status: 'skipped' })
          } else {
        ...
        if (created > 0) console.log(`  ${chalk.green('+')} ${created} file(s) created`)
        if (updated > 0) console.log(`  ${chalk.yellow('!')} ${updated} file(s) updated`)
        if (skipped > 0) console.log(`  ${chalk.dim('=')} ${skipped} file(s) skipped`)
        if (conflictCount > 0) {
