...
export interface GenerateKeyOptions {
  name: string
  permissions?: string[]
  expires?: string // e.g., "30d", "90d", "1y", "never"
  userId?: string
  metadata?: Record<string, unknown>
}
 ...
 */
export function generateApiKey(options: GenerateKeyOptions): {
  key: string
  keyId: string
  record: ApiKeyRecord
} {
...
function parseExpiration(expires?: string): number | null {
  ...
  return Date.now() + num * ms[unit as keyof typeof ms]
}
 ...
 */
export function isKeyExpired(record: ApiKeyRecord): boolean {
  if (!record.expiresAt) {
    return false
  }
...
export const keysCommands = {
   ...
   * Revoke a key
   */
  revoke: async (args: { keyId: string; json?: boolean }) => {
    if (args.json) {
   ...
   */
  info: async (args: { keyId: string; json?: boolean }) => {
    if (args.json) {
   ...
   */
  rotate: async (args: { keyId: string; json?: boolean }) => {
