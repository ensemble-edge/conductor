...
interface RunOptions {
  ...
  verbose?: boolean
  json?: boolean
  [key: string]: unknown // Allow dynamic options
}
...
export function createRunCommand(): Command {
  const run = new Command('run')
    ...
    .action(async (command: string | undefined, options: RunOptions, cmd: Command) => {
      try {
 ...
 */
function createLocalContext(): ExecutionContext {
  return {
