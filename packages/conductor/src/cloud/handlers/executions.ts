...
export async function handleExecutions(request: Request, _env: CloudEnv): Promise<Response> {
  ...
  const mockExecutions: CloudExecution[] = [
    {
      id: 'exec_stub_001',
  ...
  const response: CloudExecutionsResponse = {
    total,
    limit: query.limit!,
    offset: query.offset!,
    items,
