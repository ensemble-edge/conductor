 ...
 */
function getNamespace(componentType: ComponentType): string {
 ...
 */
function buildComponentKey(name: string, version: string, componentType: ComponentType): string {
 ...
 * @internal
 */
async function getComponentKeyWithFallback(
  name: string,
  version: string,
...
export async function loadComponent(
  ...
  if (!kv) {
    throw new Error(
      `Components KV not configured. Add ${COMPONENTS_KV_BINDING} binding to wrangler.toml:\n` +
  ...
  if (content === null) {
    const namespace = getNamespace(componentType)
    throw new Error(
      `Component not found: ${componentRef} (looked in ${namespace}/${name}/${version}). ` +
 ...
 * ```
 */
export async function loadComponentWithMetadata(
  componentRef: string,
  env: ConductorEnv,
  componentType: ComponentType = 'prompt'
  ...
  // Try type-specific namespace with legacy fallback
  const typeSpecificKey = buildComponentKey(name, version, componentType)
  let result = await kv.getWithMetadata<ComponentMetadata>(typeSpecificKey)

  // Try legacy namespace if type-specific not found
  if (result.value === null && componentType !== 'component') {
    const legacyKey = `components/${name}/${version}`
    result = await kv.getWithMetadata<ComponentMetadata>(legacyKey)
  }
 ...
 * ```
 */
export async function loadMemberConfig(memberRef: string, env: ConductorEnv): Promise<AgentConfig> {
  ...
  if (yamlContent === null) {
    throw new Error(
      `Agent config not found: ${memberRef}. ` +
  ...
  const promptRef = config.config?.prompt as string | undefined
...
export async function loadComponentMetadata(
  ...
  // Try type-specific namespace with legacy fallback
  const typeSpecificKey = buildComponentKey(name, version, componentType)
  let result = await kv.getWithMetadata<ComponentMetadata>(typeSpecificKey, { type: 'text' })

  // Try legacy namespace if type-specific not found
  if (result.value === null && componentType !== 'component') {
    const legacyKey = `components/${name}/${version}`
    result = await kv.getWithMetadata<ComponentMetadata>(legacyKey, { type: 'text' })
  }
...
export async function getComponentDeployment(
  ...
  let result = await kv.getWithMetadata<ComponentMetadata & { version?: string }>(key)

  // Try legacy namespace if not found
  if (result.value === null && componentType !== 'component') {
    const legacyKey = `components/${componentName}/${deployment}`
    result = await kv.getWithMetadata<ComponentMetadata & { version?: string }>(legacyKey)
  }
