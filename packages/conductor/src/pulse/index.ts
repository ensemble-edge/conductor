...
/** Conductor version - injected at build time */
declare const __CONDUCTOR_VERSION__: string
const CONDUCTOR_VERSION =
  typeof __CONDUCTOR_VERSION__ !== 'undefined' ? __CONDUCTOR_VERSION__ : '0.0.0-dev'
 ...
 * 4. Default → enabled
 */
export function isPulseEnabled(config: ConductorConfig, env: PulseEnv): boolean {
 ...
 */
function getProjectId(config: ConductorConfig): string {
...
export async function sendPulse(
  ...
  const sendSignal = async (): Promise<PulseResult> => {
    try {
