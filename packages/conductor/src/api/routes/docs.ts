 ...
 */
docs.get('/', async (c: ConductorContext) => {
  try {
 ...
 */
docs.get('/agents', async (c: ConductorContext) => {
  try {
    const theme = getTheme(c.env)
    const memberLoader = getMemberLoader()
    const builtInRegistry = getBuiltInRegistry()
 ...
 */
docs.get('/agents/:name', async (c: ConductorContext) => {
 ...
 */
docs.get('/ensembles', async (c: ConductorContext) => {
  try {
    const theme = getTheme(c.env)
    const ensembleLoader = getEnsembleLoader()

    const ensembles: Array<{
      name: string
      description: string
 ...
 */
docs.get('/ensembles/:name', async (c: ConductorContext) => {
  const name = c.req.param('name')
  ...
  try {
    ...
    const steps = Array.isArray(config.flow)
      ? config.flow.map((step: any, index: number) => ({
          index,
 ...
 */
docs.get('/api', async (c: ConductorContext) => {
  try {
 ...
 */
docs.get('/openapi.json', async (c: ConductorContext) => {
  try {
    const memberLoader = getMemberLoader()
    const ensembleLoader = getEnsembleLoader()
    const builtInRegistry = getBuiltInRegistry()
    ...
    const spec = generateOpenAPISpec({
      ...
      customAgents:
        memberLoader?.getMemberNames().map((n) => memberLoader.getAgentConfig(n)!) || [],
...
function generateOpenAPISpec(data: {
  ...
  for (const agent of allAgents) {
    ...
    paths[`/api/v1/execute/agent/${agentName}`] = {
      post: {
        ...
        description:
          agent.description || `Execute the ${agentName} agent${isBuiltIn ? ' (built-in)' : ''}`,
        ...
        responses: {
          ...
          '403': {
            description:
              'Forbidden - direct agent execution disabled or missing required permission',
          },
  ...
  return {
    ...
    info: {
      ...
      version: '1.0.0',
      description:
        'Auto-generated API documentation for your Conductor project. Conductor is an agentic workflow orchestration framework for Cloudflare Workers.',
    },
    servers: [
      {
        url: '/',
    ...
    tags: [
      {
        name: 'Execute',
        description:
          'Execute agents and ensembles. Requires authentication (Bearer token or API key).',
      },
      {
        name: 'Agents',
 ...
 */
docs.get('/:slug', async (c: ConductorContext) => {
