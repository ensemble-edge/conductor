 ...
 * SMS Agent Tests
 *
 * Comprehensive tests for SMS agent functionality including:
...
import { SmsMember } from '../sms-agent.js'
import type {
  SmsMemberInput,
  BatchSmsInput,
  SmsMemberOutput,
...
describe('SmsMember', () => {
  ...
  describe('Template Rendering', () => {
    it('should render simple variable substitution', async () => {
      ...
      const input: BatchSmsInput = {
        recipients: [{ phone: '+12025551234', data: { name: 'John', balance: 100 } }],
    ...
    it('should prioritize recipient data over commonData', async () => {
      ...
      const input: BatchSmsInput = {
        recipients: [{ phone: '+12025551234', data: { discount: 30 } }],
  ...
  describe('Message Length Validation', () => {
    it('should accept messages within SMS length limits', async () => {
      ...
      const result = await agent['run']({ ...mockContext, input: shortMessage })
    ...
    it('should accept concatenated messages up to 1600 characters', async () => {
      ...
      const result = await agent['run']({ ...mockContext, input: longMessage })
      expect(result).toBeDefined()
    })
    ...
    it('should handle messages over 160 characters (concatenated SMS)', async () => {
      ...
      const result = await agent['run']({ ...mockContext, input: message })
      expect(result).toBeDefined()
    })
  })
  ...
  describe('Single SMS Sending', () => {
    ...
    it('should support MMS with media URLs', async () => {
      ...
      expect(mockProvider.sentMessages[0].mediaUrl).toEqual(['https://example.com/image.jpg'])
    })
    ...
    it('should throw error when SMS send fails', async () => {
      ...
      await expect(agent['run']({ ...mockContext, input })).rejects.toThrow('SMS send failed')
    })
  })
  ...
  describe('Batch SMS Sending', () => {
    ...
    it('should track failed messages in batch', async () => {
      ...
      ;(agent as any).provider = failingProvider
    ...
    it('should continue batch send even if some messages fail', async () => {
      ...
      ;(agent as any).provider = intermittentProvider
  ...
  describe('Provider Integration', () => {
    it('should detect batch vs single SMS based on input', async () => {
      ...
      const singleResult = await agent['run']({
        ...mockContext,
        input: singleInput,
      })
      ...
      const batchResult = await agent['run']({
        ...mockContext,
        input: batchInput,
      })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle undefined/null values in template variables', async () => {
      ...
      const input: BatchSmsInput = {
        recipients: [{ phone: '+12025551234', data: { name: null, age: undefined } }],
  ...
  describe('Rate Limiting', () => {
    it('should use default rate limit of 10 SMS/sec', async () => {
      ...
      const agent = new SmsMember(config)
      expect((agent as any).rateLimit).toBe(10)
    })
    ...
    it('should use custom rate limit', async () => {
      ...
      const agent = new SmsMember(config)
      expect((agent as any).rateLimit).toBe(5)
    })
    ...
    it('should properly space messages according to rate limit', async () => {
      ...
      ;(agent as any).provider = trackingProvider
