...
npm run build   # Vite plugin scans pages/ and generates everything
...
The `npm run dev` command runs two processes in parallel:
...
**When using `vite dev` (via `npm run dev`):**
...
**With either `npm run dev` or `wrangler dev`:**
...
**Solutions:**
1. Check if you're using `npm run dev` (not `wrangler dev`)
2. Look for console output: `📄 Page change: your-page/page.yaml`
3. If not using `npm run dev`, manually run: `touch src/index.ts`
...
# Check logs
npm run dev  # Look for page registration messages
```
