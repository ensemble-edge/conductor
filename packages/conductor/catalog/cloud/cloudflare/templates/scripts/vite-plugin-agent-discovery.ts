  ...
  return {
    ...
    load(id) {
      if (id === RESOLVED_VIRTUAL_MODULE_ID) {
        const code = generateAgentsModule(root, agentsDir, fileExtension, excludeDirs);
        return code;
      }
    },
    ...
    handleHotUpdate({ file, server }) {
      if (file.includes(agentsDir) && (file.endsWith(fileExtension) || file.endsWith('.ts'))) {
...
`;
  ...
  const agentFiles = globSync(`**/*${fileExtension}`, {
    cwd: agentsDirPath,
