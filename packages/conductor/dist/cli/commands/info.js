 ...
 */
async function parseSecuritySettings() {
    const settings = {
 ...
 */
async function parseCloudflareServices() {
    const services = {
    ...
    try {
        ...
        // KV Namespaces
        const kvMatches = content.matchAll(/^\[\[kv_namespaces\]\][\s\S]*?binding\s*=\s*["']([^"']+)["']/gm);
        const kvBindings = [];
        ...
        // D1 Databases
        const d1Matches = content.matchAll(/^\[\[d1_databases\]\][\s\S]*?binding\s*=\s*["']([^"']+)["']/gm);
        const d1Bindings = [];
        ...
        // R2 Buckets
        const r2Matches = content.matchAll(/^\[\[r2_buckets\]\][\s\S]*?binding\s*=\s*["']([^"']+)["']/gm);
        const r2Bindings = [];
 ...
 */
async function parseDeploymentSettings() {
    const deployment = {
        cloudEnabled: false,
        pulse: true,
    };
 ...
 */
function formatSecuritySetting(value, explicit, defaultValue) {
 ...
 */
function displayFullStatus(status) {
    console.log('');
    ...
    console.log(`  Environment:      ${chalk.cyan(status.environment)}${status.environment === 'development' ? chalk.dim(' (local)') : ''}`);
    console.log('');
    // Configuration section
    console.log(chalk.bold('Configuration'));
    console.log(`  conductor.config: ${status.config.conductorConfig ? chalk.green('✓ Found') : chalk.dim('○ Not found')}`);
    console.log(`  wrangler.toml:    ${status.config.wranglerConfig ? chalk.green('✓ Found') : chalk.dim('○ Not found')}`);
    console.log('');
    // Components section
    console.log(chalk.bold('Components'));
    const otherCount = status.components.prompts +
    ...
    console.log(`  Agents:           ${status.components.agents.total}${status.components.agents.total > 0 ? chalk.dim(` (${status.components.agents.custom} custom)`) : ''}`);
    ...
    console.log(`  requireAuth:      ${formatSecuritySetting(status.security.requireAuth, status.security.requireAuthExplicit, true)}`);
    console.log(`  stealthMode:      ${formatSecuritySetting(status.security.stealthMode, status.security.stealthModeExplicit, false)}`);
    console.log(`  allowDirectAgentExecution: ${formatSecuritySetting(status.security.allowDirectAgentExecution, status.security.allowDirectAgentExecutionExplicit, true)}`);
    console.log('');
    // Deployment section
    ...
    console.log(`  Ensemble Cloud:   ${status.deployment.cloudEnabled ? chalk.green('✓ Connected') : chalk.dim('○ Not connected')}`);
    console.log(`  Pulse:            ${status.deployment.pulse ? chalk.green('✓ Enabled') : chalk.dim('○ Disabled')}`);
    console.log('');
 ...
 */
function displayCompactStatus(status) {
    console.log('');
    console.log(chalk.bold.cyan('Conductor Status'));
    console.log('');
    ...
    console.log(`Config:      ${status.config.conductorConfig ? chalk.green('✓') : chalk.dim('○')} conductor.config ${status.config.wranglerConfig ? chalk.green('✓') : chalk.dim('○')} wrangler.toml`);
    console.log(`Components:  ${status.components.agents.total} agents, ${status.components.ensembles} ensembles`);
    console.log(`Security:    requireAuth=${status.security.requireAuth} stealthMode=${status.security.stealthMode}`);
    console.log('');
}
// ─────────────────────────────────────────────────────────────────────────────
// Command
...
export function createInfoCommand() {
    ...
    info
        ...
        try {
            ...
            const [project, environment, components, triggers, security, plugins, cloudflare, deployment,] = await Promise.all([
                readProjectInfo(),
                detectEnvironment(),
                countComponents(),
                countTriggers(),
