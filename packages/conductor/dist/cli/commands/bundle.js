...
async function bundleAgent(agentPath, _options) {
    ...
    const agentContent = await fs.readFile(agentPath, 'utf-8');
    const agentConfig = YAML.parse(agentContent, { mapAsMap: false, logLevel: 'silent' });
    // Validate agent
    ...
    bundle
        ...
        try {
            ...
            if (!isEnsemble && !isAgent) {
                console.error(chalk.red('Error:'), 'File does not appear to be a valid ensemble or agent YAML');
                process.exit(1);
            }
            console.log('');
            console.log(chalk.bold(`Bundling ${isEnsemble ? 'ensemble' : 'agent'}: ${parsed.name || 'unknown'}`));
            ...
            const outputPath = options.output || path.join(process.cwd(), `${manifest.name}.bundle${ext}`);
            // Create archive
            console.log('');
