...
function isPrivateOrInternalIP(hostname) {
    ...
    if (lowerHostname.startsWith('fc') || lowerHostname.startsWith('fd'))
        return true;
    // fe80::/10 - Link-local
    if (lowerHostname.startsWith('fe8') ||
 ...
 */
export function validateURL(urlString, allowInternal = false) {
    let url;
    try {
    ...
    if (!allowInternal && isPrivateOrInternalIP(url.hostname)) {
        throw new Error(`SSRF protection: Blocked request to private/internal address: ${url.hostname}. ` +
