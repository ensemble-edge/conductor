...
export class HITLState extends DurableObject {
    ...
    async handleApprove(body) {
        ...
        if (this.state.status !== 'pending') {
            return new Response(JSON.stringify({ error: `Cannot approve: status is ${this.state.status}` }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        // Update state
    ...
    async handleReject(body) {
        ...
        if (this.state.status !== 'pending') {
            return new Response(JSON.stringify({ error: `Cannot reject: status is ${this.state.status}` }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        // Update state
