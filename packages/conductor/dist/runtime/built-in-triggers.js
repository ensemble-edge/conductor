...
async function handleHTTPTrigger(context) {
    ...
    if (trigger.auth) {
        try {
            ...
            logger.error(`[HTTP] Failed to create auth middleware for ${ensemble.name}:`, error instanceof Error ? error : undefined);
            return;
        }
    }
