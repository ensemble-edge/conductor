 ...
 * ```
 */
import { isAgentStep, isParallelStep, isBranchStep, isForeachStep, isTryStep, isSwitchStep, isWhileStep, isMapReduceStep, } from '../primitives/types.js';
 ...
 */
export function hasControlFlowSteps(flow) {
    return flow.some((step) => isParallelStep(step) ||
        isBranchStep(step) ||
        isForeachStep(step) ||
        isTryStep(step) ||
        isSwitchStep(step) ||
...
export class GraphExecutor {
    ...
    async execute(flow, initialContext) {
        ...
        try {
            ...
            return Result.err(new EnsembleExecutionError(this.ensembleName, 'graph-execution', error instanceof Error ? error : new Error(String(error))));
        }
    }
    /**
    ...
    async executeMapReduce(step, context) {
        ...
        reduceContext.results = mapResults; // Alias
    ...
    evaluateJsExpression(expression, context) {
        try {
            ...
            return Boolean(func(evalContext, context.input, context.state || {}, Object.fromEntries(context.results)));
        }
        catch (error) {
