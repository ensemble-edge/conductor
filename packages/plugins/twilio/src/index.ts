 ...
 */
async function handleTwilioSMSTrigger(context: TriggerHandlerContext): Promise<void> {
  const { app, ensemble, trigger, agents, env, ctx } = context
  const config = trigger as TwilioSMSTriggerConfig
...
<Response>
  <Message>${escapeXml(replyMessage)}</Message>
      ...
      logger.error(
        `[Twilio SMS] Failed to process SMS for ${ensemble.name}`,
 ...
 */
export function registerTwilioSMSTrigger(): void {
  ...
  registry.register(handleTwilioSMSTrigger, {
    ...
    schema: TwilioSMSTriggerSchema as any,
    requiresAuth: true,
