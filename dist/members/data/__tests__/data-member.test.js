 ...
 * Data Member Tests
 *
 * Comprehensive tests for Data member functionality including:
...
import { DataMember } from '../../data-member.js';
...
describe('DataMember', () => {
    ...
    describe('Configuration Validation', () => {
        it('should throw error if storage type is missing', () => {
            ...
            expect(() => new DataMember(config)).toThrow('requires storage type');
        });
        it('should throw error if operation is missing', () => {
            ...
            expect(() => new DataMember(config)).toThrow('requires operation type');
        });
        it('should accept valid configuration', () => {
            ...
            const member = new DataMember(config, mockRepository);
            expect(member).toBeDefined();
            expect(member.name).toBe('test-data');
        });
    });
    describe('GET Operation', () => {
        ...
        it('should throw error if key is missing', async () => {
            ...
            const member = new DataMember(config, mockRepository);
            const input = {};
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('GET operation requires "key"');
        });
    });
    describe('PUT Operation', () => {
        ...
        it('should throw error if key or value is missing', async () => {
            ...
            const member = new DataMember(config, mockRepository);
            // Missing value
            await expect(member['run']({ ...mockContext, input: { key: 'test' } })).rejects.toThrow('PUT operation requires "key" and "value"');
            // Missing key
            await expect(member['run']({ ...mockContext, input: { value: 'data' } })).rejects.toThrow('PUT operation requires "key" and "value"');
        });
    });
    describe('DELETE Operation', () => {
        ...
        it('should throw error if key is missing', async () => {
            ...
            const member = new DataMember(config, mockRepository);
            const input = {};
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('DELETE operation requires "key"');
        });
    });
    describe('LIST Operation', () => {
        ...
        it('should filter by prefix', async () => {
            ...
            expect(result.items.every((item) => item.key && item.key.startsWith('user:'))).toBe(true);
        });
        ...
        it('should combine prefix and limit', async () => {
            ...
            const member = new DataMember(config, mockRepository);
            const input = { prefix: 'product:', limit: 1 };
            const result = (await member['run']({
                ...mockContext,
                input
            }));
    ...
    describe('QUERY Operation', () => {
        ...
        it('should query with filter', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.items).toHaveLength(2);
            expect(result.items.every((item) => item.city === 'NYC')).toBe(true);
        });
    ...
    describe('EXPORT Operation', () => {
        beforeEach(async () => {
            ...
            await mockRepository.put('product:2', { id: 2, name: 'Gadget', price: 25.50, stock: 30 });
            await mockRepository.put('product:3', { id: 3, name: 'Doohickey', price: 15.00, stock: 100 });
        });
        ...
        it('should apply filter before export', async () => {
            ...
            await mockRepository.put('product:4', { id: 4, name: 'Thingamajig', price: 5.00, stock: 200 });
            const member = new DataMember(config, mockRepository);
            const input = {
                prefix: 'product:',
                filter: { stock: 50 },
    ...
    describe('Storage Backend Integration', () => {
        it('should use default binding name for KV', () => {
            ...
            const member = new DataMember(config, mockRepository);
            const dataConfig = member.getDataConfig();
    ...
    describe('Error Handling', () => {
        it('should handle repository errors gracefully', async () => {
            ...
            const member = new DataMember(config, errorRepo);
        ...
        it('should throw error for unknown operation', async () => {
            ...
            const member = new DataMember(config, mockRepository);
            await expect(member['run']({ ...mockContext, input: {} })).rejects.toThrow('Unknown operation');
        });
    });
    describe('Repository Pattern Integration', () => {
        it('should accept injected repository', async () => {
            ...
            const member = new DataMember(config, customRepo);
