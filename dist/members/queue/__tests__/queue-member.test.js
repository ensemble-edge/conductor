 ...
 * Queue Member Tests
 *
 * Comprehensive tests for Queue member functionality including:
...
import { QueueMember } from '../queue-member.js';
...
describe('QueueMember', () => {
    ...
    describe('Configuration Validation', () => {
        ...
        it('should accept valid configuration', () => {
            ...
            const member = new QueueMember(config);
            expect(member).toBeDefined();
            expect(member.name).toBe('test-queue');
        });
    });
    describe('Send Single Message', () => {
        ...
        it('should throw error if message is missing', async () => {
            ...
            const member = new QueueMember(config);
            const input = {};
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('Send mode requires message in input');
        });
    ...
    describe('Send Batch Messages', () => {
        ...
        it('should throw error if messages array is empty', async () => {
            ...
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('requires messages array');
        });
    ...
    describe('Message Consumption', () => {
        ...
        it('should throw error if consumer config is missing', async () => {
            ...
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('requires consumer configuration');
        });
    });
    describe('Retry Logic', () => {
        ...
        it('should calculate exponential backoff correctly', () => {
            ...
            const member = new QueueMember(config);
            // Test private method via member instance
            const delay0 = member['calculateBackoff'](0);
            const delay1 = member['calculateBackoff'](1);
            const delay2 = member['calculateBackoff'](2);
            const delay3 = member['calculateBackoff'](3);
        ...
        it('should respect max delay', () => {
            ...
            const member = new QueueMember(config);
            const delay5 = member['calculateBackoff'](5); // Would be 32000 without cap
        ...
        it('should add jitter to backoff', () => {
            ...
            const delays = Array.from({ length: 10 }, () => member['calculateBackoff'](1));
    ...
    describe('Queue Binding', () => {
        it('should throw error if queue binding not found', async () => {
            ...
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('Queue binding "NONEXISTENT_QUEUE" not found');
        });
        it('should use correct queue binding', async () => {
            const secondQueue = new MockQueue();
            mockContext.env.SECOND_QUEUE = secondQueue;
            const config = {
                name: 'second-queue',
                type: 'Queue',
                queue: 'SECOND_QUEUE',
    ...
    describe('Error Handling', () => {
        it('should handle unknown mode gracefully', async () => {
            ...
            await expect(member['run']({ ...mockContext, input })).rejects.toThrow('Unknown queue mode');
        });
