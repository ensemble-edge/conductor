 ...
 * Docs Member Tests
 *
 * Comprehensive tests for Docs member functionality including:
...
import { DocsMember } from '../docs-member.js';
...
describe('DocsMember', () => {
    ...
    describe('Configuration Validation', () => {
        it('should create docs member with default configuration', () => {
            const config = {
                name: 'api-docs',
                type: 'Docs'
            };
            const member = new DocsMember(config);
        ...
        it('should set default UI framework to Stoplight', () => {
            ...
            const member = new DocsMember(config);
            const memberConfig = member['docsConfig'];
            expect(memberConfig.ui).toBe('stoplight');
        });
        it('should set default OpenAPI version to 3.1', () => {
            ...
            const member = new DocsMember(config);
            const memberConfig = member['docsConfig'];
            expect(memberConfig.openApiVersion).toBe('3.1');
        });
        it('should set default paths', () => {
            ...
            const member = new DocsMember(config);
            const memberConfig = member['docsConfig'];
            expect(memberConfig.paths).toEqual({
                docs: '/docs',
                yaml: '/openapi.yaml',
                json: '/openapi.json'
            });
        ...
        it('should accept custom UI framework', () => {
            ...
            const member = new DocsMember(config);
            const memberConfig = member['docsConfig'];
            expect(memberConfig.ui).toBe('redoc');
        });
        it('should accept OpenAPI 3.0 version', () => {
            ...
            const member = new DocsMember(config);
            const memberConfig = member['docsConfig'];
            expect(memberConfig.openApiVersion).toBe('3.0');
        });
    });
    ...
    describe('Auto-Generation Features', () => {
        ...
        it('should respect exclude paths in auto-generation', async () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].autoGenerate?.exclude).toEqual([
                '/internal/*',
                '/private/*'
            ]);
        });
        it('should respect include paths in auto-generation', async () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].autoGenerate?.include).toEqual(['/api/*', '/v1/*']);
        });
        it('should support AI enhancement configuration', async () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].autoGenerate?.useAI).toBe(true);
            expect(member['docsConfig'].autoGenerate?.aiMember).toBe('gpt-4o');
        });
    });
    describe('Caching Functionality', () => {
        it('should cache docs UI when cache is enabled', async () => {
            ...
            const result1 = (await member['run']({
                ...mockContext,
                input
            }));
            ...
            const result2 = (await member['run']({
                ...mockContext,
                input
            }));
    ...
    describe('Branding and Customization', () => {
        it('should apply custom title across all UI frameworks', async () => {
            const frameworks = ['stoplight', 'redoc', 'swagger', 'scalar', 'rapidoc'];
    ...
    describe('Route Authentication Integration', () => {
        it('should support route-level auth configuration', () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].route?.auth?.required).toBe(true);
            expect(member['docsConfig'].route?.auth?.strategies).toEqual(['api-key']);
        });
        it('should support optional authentication', () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].route?.auth?.required).toBe(false);
        });
        it('should support multiple auth strategies', () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].route?.auth?.strategies).toEqual(['api-key', 'jwt', 'oauth']);
        });
        it('should support route priority configuration', () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].route?.priority).toBe(70);
        });
        it('should support custom route methods', () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].route?.methods).toEqual(['GET', 'POST']);
        });
        it('should support deprecated auth configuration', () => {
            ...
            const member = new DocsMember(config);
            expect(member['docsConfig'].auth?.required).toBe('required');
            expect(member['docsConfig'].auth?.apiKeyLocations).toEqual(['header', 'query']);
        });
    });
    ...
    describe('Path Configuration', () => {
        ...
        it('should handle custom spec paths', async () => {
            ...
            const member = new DocsMember(config);
            // Test YAML path
            const yamlResult = (await member['run']({
                ...mockContext,
                input: {
            ...
            // Test JSON path
            const jsonResult = (await member['run']({
                ...mockContext,
                input: {
