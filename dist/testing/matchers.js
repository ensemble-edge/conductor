 ...
 */
export function toBeSuccessful(received) {
 ...
 */
export function toHaveFailed(received) {
 ...
 */
export function toHaveExecutedSteps(received, count) {
 ...
 */
export function toHaveCompletedIn(received, ms) {
 ...
 */
export function toHaveState(received, key, value) {
 ...
 */
export function toHaveUsedTokens(received, count) {
    const totalTokens = received.aiCalls.reduce((sum, call) => sum + (call.usage?.totalTokens || 0), 0);
 ...
 */
export function toHaveCostLessThan(received, dollars) {
    const totalCost = received.aiCalls.reduce((sum, call) => sum + (call.estimatedCost || 0), 0);
 ...
 */
export function toHaveOutput(received, expected) {
 ...
 */
export function toMatchOutputShape(received, shape) {
    ...
    for (const [key, expectedType] of Object.entries(shape)) {
        if (!(key in output)) {
            ...
            mismatchedTypes.push(`${key} (expected ${typeof expectedType}, got ${typeof output[key]})`);
        }
    }
