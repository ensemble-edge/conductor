...
export class EnsembleScorer {
    ...
    getRecommendations(metrics) {
        ...
        if (metrics.passRate < 0.8) {
            recommendations.push(`Pass rate is ${(metrics.passRate * 100).toFixed(0)}%. Review failing criteria.`);
        }
        ...
        if (metrics.criteriaBreakdown) {
            for (const [criterion, data] of Object.entries(metrics.criteriaBreakdown)) {
                if (data.passRate < 0.7) {
                    recommendations.push(`Criterion '${criterion}' has low pass rate (${(data.passRate * 100).toFixed(0)}%). Focus improvement efforts here.`);
                }
            }
        }
